...
def save_images(
    ...
    connect_to_url(
        driver, url, main_url, timer, first_connection_timer, scroll, urls_done
    )
...
def check_if_ignored(url, ignored, verbose):
    ...
    for ignored_url in ignored:
        if ignored_url == "":
            continue
        if (
            ignored_url[-1] == "*" and url.startswith(ignored_url[:-1])
        ) or url == ignored_url:
            is_ignore = True
            break
    return is_ignore
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Limit of images to download (default: -1)",
    )
    parser.add_argument(
        "--extension",
        metavar="\b",
